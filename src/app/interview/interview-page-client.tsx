...
type Phase = "idle" | "fetching" | "playing" | "listening" | "submitting" | "done";
...
export default function InterviewPageClient({ sessionId, lang }: Props) {
  ...
  const [questionType, setQuestionType] = useState<"code" | "image" | "other">("other");
  ...
  const BASE_URL = process.env.NEXT_PUBLIC_BASE_AI_URL ?? "https://brova-endpoints.vercel.app";
  const speechLang = lang === "en" ? "en-US" : "ar-EG";
  ...
  const startInterview = async () => {
    ...
    lastFinal.current = "";
    try {
      const res = await fetch(`${BASE_URL}/interview/next_question?session_id=${encodeURIComponent(sessionId)}`);
  ...
  const submitAnswer = async (answerText: string, code?: string, imageBlob?: Blob) => {
    ...
    setHistory((h) => [...h, { role: "user", text: answerText || (code ? "[Code Submitted]" : "[Image Submitted]") }]);
  ...
  return (
    <main className="fixed inset-0 bg-background">
      ...
      {started && todos.length > 0 && (
        ...
        >
          <h3 className="font-semibold text-sm border-b border-border/50 pb-2">Interview Plan</h3>
          <div className="flex flex-col gap-3">
            {todos.map((todo, i) => {
              const isPast = i < currentStepIndex;
              const isCurrent = i === currentStepIndex;
              return (
                <div key={i} className={`flex gap-2.5 text-sm ${isPast ? 'text-muted-foreground line-through opacity-70' : isCurrent ? 'text-primary font-medium' : 'text-foreground/80'}`}>
        ...
        <div className="mx-auto w-full max-w-3xl overflow-hidden rounded-xl border border-border/70 bg-card/80 shadow-lg backdrop-blur-md">
          <AnimatePresence mode="wait">
            {!started ? (
              ...
              >
                {!isBrowserSupported ? (
                  <p className="text-red-500 text-sm">Browser does not support Speech Recognition.</p>
                ) : (
              ...
              >
                  ...
                  <div className="border-b border-border/70 px-4 py-2">
                    <div className="flex items-center justify-between gap-3 text-xs text-muted-foreground">
                      <div className="flex items-center gap-2">
                        <span className={`size-2 rounded-full ${phase === 'playing' ? 'bg-blue-500 animate-pulse' : phase === 'listening' ? 'bg-red-500 animate-pulse' : 'bg-primary'}`} />
                        <span>
                          {phase === 'fetching' && 'Connecting...'}
                          {phase === 'playing' && 'AI is speaking...'}
                          {phase === 'listening' && questionType === 'other' && 'Listening to you...'}
                          {phase === 'listening' && (questionType === 'code' || questionType === 'image') && 'Waiting for your submission...'}
                ...
                <div className="max-h-48 overflow-y-auto px-4 py-3 flex flex-col gap-2 pointer-events-auto">
                  ...
                  {phase === "listening" && (questionType === "code" || questionType === "image") && !canvasOpen && !vadEnabled && (
                    <div className="pointer-events-auto flex flex-col gap-2 mt-4 items-end">
                      <p className="text-xs text-muted-foreground mb-1">How would you like to respond?</p>
                      <div className="flex gap-2">
                        ...
                        >
                          {questionType === "code" ? "Open Code Editor" : "Open Drawing Canvas"}
                        </button>
                      </div>
                    </div>
                  )}
